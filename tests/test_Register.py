...
class TestRegister(BaseTest):
    def test_register_with_mandatory_fields(self):
        ...
        account_success_page = register_page.register_an_account(
            ExcelUtils.get_cell_data("ExcelFiles/Data_test.xlsx","RegisterTest",2,1),
            ExcelUtils.get_cell_data("ExcelFiles/Data_test.xlsx","RegisterTest",2,2),
            self.generate_email_with_time_stamp(),
            "1234567890",
            "Suman2215@","Suman2215@","no","select")
    ...
    def test_register_with_all_fields(self):
        home_page = HomePage(self.driver)
        register_page = home_page.navigate_to_register_page()
        account_success_page = register_page.register_an_account("Suman","Chouhan",self.generate_email_with_time_stamp(),"1234567890","Suman2215@","Suman2215@","yes","select")
        expected_heading_text = "Your Account Has Been Created!"
        assert account_success_page.retrieve_account_creation_message().__eq__(expected_heading_text)
    ...
    def test_register_with_duplicate_email(self):
        home_page = HomePage(self.driver)
        register_page = home_page.navigate_to_register_page()
        register_page.register_an_account("Suman","Chouhan","sumanchouhan0313@gmail.com","1234567890","Suman2215@","Suman2215@","yes","select")
